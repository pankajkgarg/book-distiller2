...
- `pivotal`: preserve the complete central argument or model, its indispensable reasoning, the defining stories, important caveats, and enough original language for the chapter to retain its force. Give the prose comfortable pacing.
...
5. Read the draft as continuous prose. Restore any missing causal step or concrete detail whose absence makes it choppy, generic, or unpersuasive.
6. Verify that every assigned chapter has its own heading, every distinct lesson survives, and every blockquote is exact source text.
